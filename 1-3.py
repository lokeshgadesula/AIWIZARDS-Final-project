...
params = {
    'cubic': {
        'a': tf.Variable(0.0, name='a_cubic'),
        'b': tf.Variable(0.0, name='b_cubic'),
        'c': tf.Variable(0.0, name='c_cubic'),
        'd': tf.Variable(0.0, name='d_cubic')
    },
    'quartic': {
        'a': tf.Variable(0.0, name='a_quartic'),
        'b': tf.Variable(0.0, name='b_quartic'),
        'c': tf.Variable(0.0, name='c_quartic'),
        'd': tf.Variable(0.0, name='d_quartic'),
        'e': tf.Variable(0.0, name='e_quartic')
    }
}
...
# Training function
def train_step(x, y, model_params, model_type):
    with tf.GradientTape() as tape:
        if model_type == 'cubic':
            predictions = (model_params['a'] * x**3 + 
                           model_params['b'] * x**2 + 
                           model_params['c'] * x + 
                           model_params['d'])
        elif model_type == 'quartic':
            predictions = (model_params['a'] * x**4 + 
                           model_params['b'] * x**3 + 
                           model_params['c'] * x**2 + 
                           model_params['d'] * x + 
                           model_params['e'])
    ...
    gradients = tape.gradient(loss, list(model_params.values()))
...
for model_type, model_params in params.items():
    ...
    for epoch in range(epochs):
        loss = train_step(X_data, Y_data, model_params, model_type)
        if epoch % 10 == 0:
            ...
            if model_type == 'cubic':
                Y_model = (model_params['a'].numpy() * X_model**3 +
                           model_params['b'].numpy() * X_model**2 +
                           model_params['c'].numpy() * X_model +
                           model_params['d'].numpy())
            elif model_type == 'quartic':
                Y_model = (model_params['a'].numpy() * X_model**4 +
                           model_params['b'].numpy() * X_model**3 +
                           model_params['c'].numpy() * X_model**2 +
                           model_params['d'].numpy() * X_model +
                           model_params['e'].numpy())
            
            plt.plot(X_model, Y_model, color='red', label=f'Fitted Curve at Epoch {epoch}')
            plt.title(f'{model_type.capitalize()} Fitted Curve to Data Points')
            plt.xlabel('X')
            plt.ylabel('Y')
            plt.legend()
            plt.show()